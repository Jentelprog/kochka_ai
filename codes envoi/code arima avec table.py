...
# Nom de la table dont vous voulez supprimer le contenu
nom_table = "new_predictions"

# Requête SQL pour supprimer le contenu de la table
sql = f"DELETE FROM {nom_table}"
...
model = ARIMA(data_resampled['puissance'], order=(14, 0, 16))  # Exemple : ARIMA(p,d,q) avec p=11, d=0, q=14
model_fit = model.fit()
...
# Convertir la série temporelle en DataFrame avec deux colonnes : timestamp et puissance
data_to_insert = data_resampled.reset_index()

# Créer une table pour stocker les données resamplées
create_table_query = """
CREATE TABLE IF NOT EXISTS data_resampled (
...
# Insérer les données resamplées dans la table
for _, row in data_to_insert.iterrows():
    # Formatage de la date et de l'heure au format MySQL
    timestamp_mysql = row['ttime'].strftime('%Y-%m-%d %H:%M:%S')
    # Requête SQL d'insertion
    insert_query = "INSERT INTO data_resampled (timestamp, puissance) VALUES (%s, %s)"
    # Exécution de la requête
    cursor.execute(insert_query, (timestamp_mysql, row['puissance']))
