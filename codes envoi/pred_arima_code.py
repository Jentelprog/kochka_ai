...
data_clean["ttime"] = pd.to_datetime(
    data_clean["ttime"]
)  # si la colonne 'ttime' est de type datetime
data_clean.set_index("ttime", inplace=True)  # Définir 'ttime' comme index
data_resampled = data_clean.resample(
    "H"
).mean()  # Rééchantillonner les données à une fréquence horaire en prenant la moyenne
data_resampled.fillna(
    data_resampled.mean(), inplace=True
...
plot_acf(
    data_resampled["puissance"], lags=50
)  # Lags : nombre de décalages à inclure dans le calcul de l'ACF
...
plt.figure(figsize=(12, 6))
plot_pacf(
    data_resampled["puissance"], lags=22
)  # Lags : nombre de décalages à inclure dans le calcul de la PACF
...
model = ARIMA(train_data["puissance"], order=(16, 1, 19))
model_fit = model.fit()
...
plt.plot(
    test_data.index,
    test_data["puissance"],
    label="Ensemble de test (réel)",
    color="blue",
    linestyle="--",
)
plt.plot(
    train_predictions.index,
    train_predictions,
    label="Prédictions (Entraînement)",
    color="red",
)
plt.plot(
    test_predictions.index, test_predictions, label="Prédictions (Test)", color="green"
)

# Définir les limites des axes x et y pour une continuité visuelle
plt.xlim(train_data.index[0], test_data.index[-1])
plt.ylim(
    min(train_data["puissance"].min(), test_data["puissance"].min()),
    max(train_data["puissance"].max(), test_data["puissance"].max()),
)
...
train_rmse = mean_squared_error(
    train_data["puissance"], train_predictions, squared=False
)
...
plt.plot(
    data_resampled.index[-50:],
    data_resampled["puissance"].tail(50),
    label="Données historiques",
)
plt.plot(
    pd.date_range(start=data_resampled.index[-1], periods=forecast_steps, freq="H"),
    forecast,
    label="Prévisions",
)
